...
func deal_damage(dmg : int):
	hp -= dmg
	if hp <= 0:
		for i in range(firewood_amount):
			var firewood = FIREWOOD.instantiate()
			get_tree().current_scene.add_child(firewood)
			firewood.global_position = global_position + Vector3.UP*0.1*i
